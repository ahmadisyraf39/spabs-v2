...
import PlayerProgressGrid from './PlayerProgressGrid'
...
export default function PlayerProgressPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null)
      try {
        const [p, t] = await Promise.all([getPlayer(id), getTeam(teamId)])
        if (cancelled) return
        setPlayer(p)
        setTeam(t)
      } catch (err) {
    ...
    return (
      <div className="mx-auto max-w-lg">
        <div role="alert" className="alert alert-warning text-sm">
          <span>
            No team selected — go back to the player and click "View" from a team's Skills column.
          </span>
        </div>
      </div>
    )
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <PlayerProgressGrid playerId={id} teamId={teamId} />
    </div>
  )
}
